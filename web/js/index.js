...
function ca(obj){   
    ...
    for (var i = 0; i < max; i++) { 
        ...
        if(filename != ''){
            if(checkone == 1 && $('#editable li').length > 0){

            }else{
                $('#editable').prepend('<li><input type="hidden" name="'+d_postimg+'[]" value="'+filename+'" /><img src="'+sources[i]+'"><i class="js-remove">✖</i></li>');
            }
        }
    };        
...
function clickbackparent(element) {    
    ...
    if(parentClass == 'tab-pane fade in active' || parentClass == 'tab-pane fade' || parentClass == 'tab-pane fade in ') 
...
function selectmur(id) { 
    $('#'+id+'.mulr').each (function () {
        ...
        $(this).after('<div id="'+idpa+'-pa" class="mulr-pa '+showradio+' " '+dataplacement + datatrigger + dataoriginaltitle +' '+d_i+'="'+idpj+'" '+d_t+'="'+dt+'"  '+d_s+'="'+ds+'" '+d_c+'="'+dc+'" '+d_ty+'="'+type+'" data-html="true" data-toggle="tooltip"></div>');       
...
function headertable(pjid) {
    ...
    if($('#pj'+pjid).parents('.modal').length < 1){
